...
// Note: don't import jQuery at module scope (server-side errors).
...
const useSticky = () => {
  ...
  function adjustMenuBackground() {
    ...
    // Lazy-load jQuery only on the client when needed to avoid
    // "jQuery requires a window with a document" during SSR.
