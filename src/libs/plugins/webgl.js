...
export default class WebGL {
  ...
  addObjects() {
    ...
    // );
    const sliderHolder = document.getElementById("showcase-slider-holder");

    const patternImage = sliderHolder?.getAttribute("data-pattern-img");

    if (!patternImage) {
      throw new Error(
        'Missing "data-pattern-img" attribute on #showcase-slider-holder',
      );
    }

    const texture = new THREE.TextureLoader().load(patternImage);
